...
impl Monitor {
    ...
    pub fn load_configuration(&mut self, configs: BTreeMap<String, Configuration>) {
        ...
        self.logger
            .log_with_prefix("Monitor", "Configuration loading has been initiated");
            ...
            match tasks.entry(task_name.clone()) {
                Entry::Vacant(entry) => {
                    self.logger.log_with_prefix(
                        "Monitor",
                        &format!("New task: {task_name} has been added"),
                    );
                ...
                Entry::Occupied(mut entry) => {
                    if entry.get()[0].configuration != update.configuration {
                        ...
                        self.logger.log_with_prefix(
                            "Monitor",
                            &format!("Existing task: {task_name} wasn't modified"),
                        );
                    }
                }
            }
        ...
        return match task_name {
            ...
            Some(ref task_name) => match tasks.get(task_name.as_str()) {
                None => {
                    self.logger.log_with_prefix(
                        "Monitor",
                        &format!("Task status: {task_name} wasn't found"),
                    );
                ...
                Some(task) => {
                    self.logger.log_with_prefix(
                        "Monitor",
                        &format!("Task status: {task_name} returning status"),
                    );
                    format!(
                        "{task_name}: {}",
                        task.iter()
        ...
        for (task_name, task) in tasks.iter_mut() {
            for (i, process) in task.iter_mut().enumerate() {
                if process.configuration.auto_start {
                    self.logger.log_with_prefix(
                        "Monitor",
                        &format!("Autostarting {task_name} #{}", i + 1),
                    );
                    let _ = process.run();
                }
            }
    ...
    fn get_task_json_config_by_name(&self, name: &String) -> Option<String> {
            ...
            .map(|task| task[0].get_json_configuration())
        {
            None => {
                self.logger
                    .log_with_prefix("Monitor", &format!("Get config: {name} wasn't found"));
                None
            }
            Some(config) => {
                self.logger.log_with_prefix(
                    "Monitor",
                    &format!("Get config: returning {name} configuration in json format"),
                );
                Some(config)
            }
        };
    ...
    fn stop_task_by_name(&mut self, name: &String) -> Result<(), String> {
        let mut tasks = self.tasks.lock().unwrap();
        self.logger
            .log_with_prefix("Monitor", &format!("Stop task: stopping {name}..."));
        if let Some(task) = tasks.get_mut(name) {
            for (i, process) in task.iter_mut().enumerate() {
                if let Err(e_msg) = process.stop() {
                    self.logger.log_with_prefix(
                        "Monitor",
                        &format!("Stop task: can't stop {name} #{}: {e_msg}", i + 1),
                    );
            ...
            self.logger
                .log_with_prefix("Monitor", &format!("Stop task: {name} wasn't found"));
    ...
    fn start_task_by_name(&mut self, name: &String) -> Result<(), String> {
        let mut tasks = self.tasks.lock().unwrap();
        self.logger
            .log_with_prefix("Monitor", &format!("Start task: starting {name}..."));
        if let Some(task) = tasks.get_mut(name) {
            for (i, process) in task.iter_mut().enumerate() {
                if let Err(e_msg) = process.run() {
                    self.logger
                        .log_with_prefix("Monitor", &format!("Start task: can't start {name}..."));
